...
class UserController extends Controller {
	...
	public function productLine() {
		...
		$lists = [];
		if ($product_lists) {
			$list['cid'] = $product_lists[0]->cid;
			$lists['image'] = $product_lists[0]->image;
			foreach ($product_lists as $list) {
				$lists['data'][] = $list;
			}
		}
		return jsonAjax(StatusNo::SUCCESS, StatusNo::getStatusMsg(StatusNo::SUCCESS), $lists);
	}
